...
class TestApplyTilePredictions:
    def _make_prediction(self, base_terrain: int = 2) -> list[int]:
        """Minimal 18-feature vector: only baseTerrain set, rest default/0."""
        feat = [0] * 18
...
class TestLastRevealedTiles:
    ...
    def test_features_length_is_18(self) -> None:
        """Each feature vector must have exactly 18 elements."""
        env = _env(seed=13)
        env.step_fast(env.legal_action_ids_fast()[0])
