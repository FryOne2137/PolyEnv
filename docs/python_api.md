...
For hidden-map experiments, `hidden_action_targets()` and
`clone_with_predictions()` are available from `PolyEnv`. They are optional;
normal policies should use only the player-view packet.
